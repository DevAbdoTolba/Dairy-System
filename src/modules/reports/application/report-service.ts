...
export function getReport(from: string, to: string) {
  const transactions = listTransactions({ from, to, includeVoided: false, limit: 1000 });
  const inventory = getInventorySummary(from, to);
...
export function reportCsv(from: string, to: string) {
  const { transactions } = getReport(from, to);
  const rows = [
